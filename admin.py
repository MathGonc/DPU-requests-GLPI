...
xpathRequestLoading = '//*[@id="divCorpoJanelaAguarde_JANELA_AGUARDE_MENU"]/table/tbody/tr/td'
...
def verifyRequestIsAuto(requestId):
    ...
    for path in os.listdir(config.request_path):
        ...
        if os.path.isfile(os.path.join(config.request_path, path)):
            count += 1
            file = open(config.request_path + '/' + str(count) + '.txt', mode="r", encoding="utf-8")
            request = file.readlines()

            requestText = config.driver.find_element(By.XPATH,'//*[@id="list-item-' + requestId + '"]/div[10]').get_property("innerText")
...
def closeRequest():
    if config.request_manual == 0:
        time.sleep(config.sleeptime)
        requestList = config.driver.find_elements(By.XPATH, "//div[contains(@class, 'request-id ng-binding')]")
        ...
        for r in range(len(requestList)):
            ...
            if verifyRequestIsAuto(requestNumber) == 1:
                ...
                print('request ' + requestNumber + ' is not automated, looking for another...')

                if r == (len(requestList)-1):
                    config.request_number = requestNumber
                    config.request_manual = 1
                    print('no automated request found, opening ['+ requestNumber + '] in manual mode...')
    ...
    WebDriverWait(config.driver, 60).until(EC.presence_of_element_located((By.XPATH, xpathPageRequestReview)))
    ...
    WebDriverWait(config.driver, 60).until(EC.presence_of_element_located((By.XPATH, '//*[@id="radio-request-resolvida"]')))
    time.sleep(config.sleeptime)
    ...
    match (random.randint(0,2)):
        case 0: nameUser = nameUser
        case 1: nameUser = nameUser.split()[0]
        case 2:
            if len(nameUser.split()[1]) > 3: #case second part of name no has 'de', 'da', 'dos'
    ...
    #Patrimonio
    WebDriverWait(config.driver, 60).until(EC.presence_of_element_located((By.XPATH, xpathRequestPatrominio)))
    patrimonio = config.driver.find_element(By.XPATH, xpathRequestPatrominio).get_property("value")
    ...
    WebDriverWait(config.driver, 60).until(EC.presence_of_element_located((By.XPATH, xpathSolution)))
    elementSolution = config.driver.find_element(By.XPATH, xpathSolution)

    # Input category cause
    listClassCause = ['Hardware', 'Configuration', 'Outros', 'Software', 'Erro de aplicação', 'Erro de configuação']
    config.request_class_cause = listClassCause[random.randint(0, (len(listClassCause) - 1))]

    config.driver.find_element(By.XPATH, '// *[ @ id = "cause"] / div[1] / span / span[2]').click()
    config.driver.find_element(By.XPATH, '//*[@id="cause"]/input[1]').send_keys(config.request_class_cause)
    time.sleep(config.sleeptime)
    ActionChains(config.driver).send_keys(Keys.RETURN).perform()

    # Input category solution
    listClassSolution = ['Ajuste na configuração do aplicativo', 'Ajustes de configurações do sistema', 'Hardware', 'Software', 'Ajuste de configuração de software']
    config.request_class_solution = listClassSolution[random.randint(0, (len(listClassSolution) - 1))]

    config.driver.find_element(By.XPATH, '//*[@id="solution-category"]/div[1]/span/span[2]').click()
    config.driver.find_element(By.XPATH, '//*[@id="solution-category"]/input[1]').send_keys(config.request_class_solution)
    time.sleep(config.sleeptime)
    ActionChains(config.driver).send_keys(Keys.RETURN).perform()
    ...
    if config.request_manual == 1:
        elementSolution.send_keys('Descrição de atendimento: Usuário reportou que XXXXXXX, problema resolvido, conforme havia sido solicitado.')
        elementSolution.send_keys(Keys.ENTER)
        elementSolution.send_keys('Testes executados: Teste realizado na presença do usuário, tudo em funcionamento.')
        elementSolution.send_keys(Keys.ENTER)
        elementSolution.send_keys('Evidências: Segue anexo.')
        elementSolution.send_keys(Keys.ENTER)
        elementSolution.send_keys(Keys.ENTER)
        elementSolution.send_keys('Sua solicitação foi atendida! Pedimos que responda nossa pesquisa de satisfação.')


        try:
            wait = WebDriverWait(config.driver,  99999)
            wait.until(EC.presence_of_element_located((By.XPATH, '//*[@id="service-request-incident-container"]/div[1]/div[1]'))) #tickets list
        finally:
            time.sleep(config.sleeptime)

    #Automatico
    else:
        elementSolution.send_keys('Descrição de atendimento: ' + config.request_solution)
        elementSolution.send_keys(Keys.ENTER)
        elementSolution.send_keys('Testes executados: Teste realizado na presença do usuário, tudo em funcionamento.')
        elementSolution.send_keys(Keys.ENTER)
        elementSolution.send_keys('Evidências: Segue anexo.')
        ...
        config.driver.find_element(By.XPATH, '// *[ @ id = "request-save-submit"]').click()  # gravar e enviar
        time.sleep(config.sleeptime)
