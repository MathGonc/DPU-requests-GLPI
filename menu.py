...
def login():
    ...
    if len(config.userLoginName) == 0:
        WebDriverWait(config.driver, 1000).until(EC.presence_of_element_located((By.XPATH, '/html/body/nav/div/div[1]/a/img')))  # logo dpu
        
        nameUser = config.driver.find_element(By.XPATH, '// *[ @ id = "navbar"] / ul / li[4] / a / span[1]').text
    ...
    for (root, dirs, file) in os.walk(cookies.pathCookie):
        for i in file:
            if cookies.extension in i:
                requestList = requestList + str(count) + ' - ' + i.replace(cookies.extension, '') + '\n'
                count += 1
    ...
    match int(inputValue):
        case 0:
            ...
            match (random.randint(0, 3)):
                case 0: config.request_link = 'https://suporte.dpu.def.br/citsmart/pages/smartPortal/smartPortal.load#/atividade/4/146/151'
                case 1: config.request_link = 'https://suporte.dpu.def.br/citsmart/pages/smartPortal/smartPortal.load#/atividade/4/146/152'
                case 2: config.request_link = 'https://suporte.dpu.def.br/citsmart/pages/smartPortal/smartPortal.load#/atividade/4/153/161'

        case _:
            # verify file request exists
            file_exists = os.path.exists(config.request_path + '/' + inputValue + '.txt')
            if file_exists == 0:
                print('request number dont exist, exiting...')
                time.sleep(config.sleeptime)
                exit()

            file = open(config.request_path + '/' + inputValue + '.txt', mode="r", encoding="utf-8")
            request = file.readlines()
            ...
            if len(config.request_patrimonio) <= 1:
                config.request_patrimonio = input('Este tipo de chamado exige um patrimonio: ')